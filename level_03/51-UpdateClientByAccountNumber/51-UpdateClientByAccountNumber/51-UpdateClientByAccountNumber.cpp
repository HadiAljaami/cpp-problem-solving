...
void WriteClientToFile(string ClientsFileName, vector <sClient>& vClients)
{
	...
	if (FileClient.is_open())
	{
		...
		{
			FileClient << ConvertRecordToLine(vClients[i]) << endl;
...
bool UpdateClientFromFile(string AccountNumber, vector <sClient> vClients)
{
	char ToUpdate = 'n';
	...
	for (int i = 0;i<vClients.size();i++)
	{
		...
		if (AccountNumber == vClients[i].AccountNumber)
		{
			...
			cout << endl;
			cout << "Do you want Update this Client (y,Y) :";
			
			cin >> ToUpdate;
			if (ToUpdate == 'y' || ToUpdate == 'Y')
			{
				vClients[i] = ChangeClient(AccountNumber);
...
int main()
{
	...
	UpdateClientFromFile(AccountNumber, vClients);
